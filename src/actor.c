...
struct actor *create_actor(int x, int y, int ch, char *name,
                           TCOD_color_t col)
{
...
struct actor *make_item(int x, int y, double power, double range,
                        ...
                        bool (*use)(struct engine *engine,
                                    ...
                                    struct actor *item))
{
    struct actor *tmp = create_actor(x, y, ch, name, col);
...
struct actor *get_actor(struct engine *engine, int x, int y)
{
    ...
    {
        struct actor *actor = *iter;
        if (actor->x == x && actor->y == y && actor->life && !is_dead(actor))
            return actor;
    }
    return NULL;
}
                                  ...
                                  double range)
{
    ...
    {
        struct actor *actor = *iter;
        if (actor != engine->player && actor->life && !is_dead(actor))
        {
...
{
    if (container->capacity > 0 && TCOD_list_size(container->items) > container->capacity)
        return false;
                        ...
                        struct actor *item)
{
    ...
    if (make_hungry(actor, item->pickable->calc_food_cost(actor, item)))
    {
        /*
...
{
    if (actor->life && actor->life->stomach - amount >= 0)
    {
                   ...
                   struct actor *target)
{
    ...
    if (reward > 0)
    {
        ...
        char *reward_message = actor == engine->player ? "%s is dead. %s gain %.0f xp.\n" : "%s is dead. %s gains %.0f xp.\n";
...
void render_actor(struct actor *actor)
{
    terminal_put(actor->x, actor->y, actor->ch);
}
