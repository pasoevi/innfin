...
void render_colors(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    ...
    static TCOD_color_t cols[4] = {{50,  40,  150},
                                   {240, 85,  5},
                                   {50,  35,  240},
                                   {10,  200, 130}}; /* random corner colors */
    static int dirr[4] = {1, -1, 1, 1}, dirg[4] = {1, -1, -1, 1}, dirb[4] = {1,
                                                                             1,
                                                                             1,
                                                                             -1};
    int c, x, y;
    ...
    for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
        ...
        TCOD_color_t top = TCOD_color_lerp(cols[TOPLEFT], cols[TOPRIGHT],
                                           xcoef);
        TCOD_color_t bottom = TCOD_color_lerp(cols[BOTTOMLEFT],
                                              cols[BOTTOMRIGHT], xcoef);
    ...
    textColor = TCOD_console_get_char_background(sample_console,
    ...
    for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
        for (y = 0; y < SAMPLE_SCREEN_HEIGHT; y++) {
            int c;
            TCOD_color_t col = TCOD_console_get_char_background(sample_console,
                                                                x, y);
    ...
    TCOD_console_print_rect_ex(sample_console, SAMPLE_SCREEN_WIDTH / 2, 5,
                               ...
                               SAMPLE_SCREEN_HEIGHT - 1, TCOD_BKGND_MULTIPLY,
                               TCOD_CENTER,
...
void render_offscreen(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    ...
    TCOD_console_blit(screenshot, 0, 0, SAMPLE_SCREEN_WIDTH,
                      SAMPLE_SCREEN_HEIGHT,
                      sample_console, 0, 0, 1.0f, 1.0f);
    /* blit the overlapping screen */
    TCOD_console_blit(secondary, 0, 0, SAMPLE_SCREEN_WIDTH / 2,
                      SAMPLE_SCREEN_HEIGHT / 2,
 ...
 * line drawing sample
 * ***************************/
static int bk_flag = TCOD_BKGND_SET;

/* current blending mode */
bool line_listener(int x, int y)
{
    if (x >= 0 && y >= 0 && x < SAMPLE_SCREEN_WIDTH &&
        y < SAMPLE_SCREEN_HEIGHT) {
        TCOD_console_set_char_background(sample_console, x, y, TCOD_light_blue,
                                         (TCOD_bkgnd_flag_t) bk_flag);
    }
    return true;
}
...
void render_lines(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    ...
    if (!init) {
        ...
        for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
            for (y = 0; y < SAMPLE_SCREEN_HEIGHT; y++) {
                ...
                col.g = (uint8) ((x + y) * 255 / (SAMPLE_SCREEN_WIDTH - 1 +
    ...
    TCOD_console_blit(bk, 0, 0, SAMPLE_SCREEN_WIDTH, SAMPLE_SCREEN_HEIGHT,
                      sample_console, 0, 0, 1.0f, 1.0f);
    /* render the gradient */
    recty = (int) ((SAMPLE_SCREEN_HEIGHT - 2) *
                   ((1.0f + cosf(TCOD_sys_elapsed_seconds())) / 2.0f));
    for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
        TCOD_color_t col;
        col.r = (uint8) (x * 255 / SAMPLE_SCREEN_WIDTH);
        col.g = (uint8) (x * 255 / SAMPLE_SCREEN_WIDTH);
        col.b = (uint8) (x * 255 / SAMPLE_SCREEN_WIDTH);
        TCOD_console_set_char_background(sample_console, x, recty, col,
                                         (TCOD_bkgnd_flag_t) bk_flag);
        TCOD_console_set_char_background(sample_console, x, recty + 1, col,
                                         (TCOD_bkgnd_flag_t) bk_flag);
        TCOD_console_set_char_background(sample_console, x, recty + 2, col,
                                         (TCOD_bkgnd_flag_t) bk_flag);
    }
    ...
    TCOD_console_print(sample_console, 2, 2, "%s (ENTER to change)",
...
void render_noise(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
        ...
        for (x = 0; x < 2 * SAMPLE_SCREEN_WIDTH; x++) {
            ...
            switch (func) {
                ...
                case WAVELET :
                    ...
                    break;
                case FBM_PERLIN :
                    value = TCOD_noise_get_fbm_ex(noise, f, octaves,
                                                  TCOD_NOISE_PERLIN);
                    break;
                case TURBULENCE_PERLIN :
                    value = TCOD_noise_get_turbulence_ex(noise, f, octaves,
                                                         TCOD_NOISE_PERLIN);
                    break;
                case FBM_SIMPLEX :
                    value = TCOD_noise_get_fbm_ex(noise, f, octaves,
    ...
    TCOD_console_set_default_background(sample_console, TCOD_grey);
    TCOD_console_rect(sample_console, 2, 2, 23, (func <= WAVELET ? 10 : 13),
                      false, TCOD_BKGND_MULTIPLY);
    for (y = 2; y < 2 + (func <= WAVELET ? 10 : 13); y++) {
        for (x = 2; x < 2 + 23; x++) {
            TCOD_color_t col = TCOD_console_get_char_foreground(sample_console,
                                                                x, y);
    ...
    for (curfunc = PERLIN; curfunc <= TURBULENCE_WAVELET; curfunc++) {
        if (curfunc == func) {
            TCOD_console_set_default_foreground(sample_console, TCOD_white);
            TCOD_console_set_default_background(sample_console,
                                                TCOD_light_blue);
            TCOD_console_print_ex(sample_console, 2, 2 + curfunc,
                                  TCOD_BKGND_SET, TCOD_LEFT, funcName[curfunc]);
        } else {
    ...
    if (func > WAVELET) {
        ...
        TCOD_console_print(sample_console, 2, 13, "R/F : lacunarity (%2.1f)",
                           lacunarity);
        TCOD_console_print(sample_console, 2, 14, "T/G : octaves (%2.1f)",
                           octaves);
    }
    /* handle keypress */
...
#define SQUARED_TORCH_RADIUS (TORCH_RADIUS*TORCH_RADIUS)
    ...
    static char *algo_names[] = {"BASIC      ", "DIAMOND    ", "SHADOW     ",
                                 ...
                                 "PERMISSIVE3", "PERMISSIVE4",
                                 "PERMISSIVE5", "PERMISSIVE6", "PERMISSIVE7",
        ...
        for (y = 0; y < SAMPLE_SCREEN_HEIGHT; y++) {
            for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
                if (smap[y][x] == ' ')
                    ...
                    TCOD_map_set_properties(map, x, y, true,
                                            false); /* window */
            }
        }
        noise = TCOD_noise_new(1, 1.0f, 1.0f,
                               NULL); /* 1d noise for the torch flickering */
    }
    if (first) {
        TCOD_sys_set_fps(30); /* limited to 30 fps */
        ...
        TCOD_console_print(sample_console, 1, 0,
                           "IJKL : move around\nT : torch fx %s\nW : light walls %s\n+-: algo %s",
                           torch ? "on " : "off", light_walls ? "on " : "off",
                           algo_names[algonum]);
    ...
    if (recompute_fov) {
        ...
        TCOD_map_compute_fov(map, px, py, torch ? (int) (TORCH_RADIUS) : 0,
                             light_walls, (TCOD_fov_algorithm_t) algonum);
    }
    if (torch) {
        float tdx;
        ...
        for (x = 0; x < SAMPLE_SCREEN_WIDTH; x++) {
            ...
            if (!visible) {
                ...
                if (!torch) {
                    ...
                    float r = (x - px + dx) * (x - px + dx) + (y - py + dy) *
                                                              (y - py +
                                                               dy); /* cell distance to torch (squared) */
                    if (r < SQUARED_TORCH_RADIUS) {
                        float l = (SQUARED_TORCH_RADIUS - r) /
                                  SQUARED_TORCH_RADIUS + di;
                        l = CLAMP(0.0f, 1.0f, l);
...
#define SQUARED_TORCH_RADIUS (TORCH_RADIUS*TORCH_RADIUS)
    ...
    if (usingAstar) {
        for (i = 0; i < TCOD_path_size(path); i++) {
            ...
            TCOD_console_set_char_background(sample_console, x, y, light_ground,
                                             TCOD_BKGND_SET);
        }
    } else {
        int x, y, i;
    ...
    if (mx >= 0 && mx < SAMPLE_SCREEN_WIDTH && my >= 0 &&
        my < SAMPLE_SCREEN_HEIGHT && (dx != mx || dy != my)) {
        TCOD_console_put_char(sample_console, dx, dy, oldChar, TCOD_BKGND_NONE);
        dx = mx;
        dy = my;
        oldChar = TCOD_console_get_char(sample_console, dx, dy);
...
void render_bsp(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    ...
    if (generate || refresh) {
        ...
        if (!bsp) {
            // create the bsp
            bsp = TCOD_bsp_new_with_size(0, 0, SAMPLE_SCREEN_WIDTH,
                                         SAMPLE_SCREEN_HEIGHT);
        } else {
            // restore the nodes size
            TCOD_bsp_resize(bsp, 0, 0, SAMPLE_SCREEN_WIDTH,
                            SAMPLE_SCREEN_HEIGHT);
        }
        memset(map, '#',
        ...
        if (generate) {
            ...
            TCOD_bsp_split_recursive(bsp, NULL, bspDepth,
                                     ...
                                     minRoomSize + (roomWalls ? 1 : 0), 1.5f,
                                     1.5f);
        }
...
void render_name(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    ...
    if (!names) {
        ...
        // parse all the files
        for (it = (char **) TCOD_list_begin(files);
             it != (char **) TCOD_list_end(files); it++) {
            char tmp[236];
    ...
    for (i = 0; i < TCOD_list_size(names); i++) {
        ...
        if (strlen(name) < SAMPLE_SCREEN_WIDTH)
            TCOD_console_print_ex(sample_console, SAMPLE_SCREEN_WIDTH - 2,
                                  2 + i,
    ...
    if (delay >= 0.5f) {
        ...
        TCOD_list_push(names, TCOD_namegen_generate(
                (char *) TCOD_list_get(sets, curSet), true));
    }
    if (key->c == '+') {
        curSet++;
...
void burn(SDL_Surface *screen, int samplex, int sampley, int samplew,
          int sampleh)
{
    ...
    for (x = samplex; x < samplex + samplew; x++) {
        Uint8 *p =
                (Uint8 *) screen->pixels + x * screen->format->BytesPerPixel +
                sampley * screen->pitch;
...
void explode(SDL_Surface *screen, int samplex, int sampley, int samplew,
             int sampleh)
{
    int ridx = screen->format->Rshift / 8;
    int gidx = screen->format->Gshift / 8;
...
void blur(SDL_Surface *screen, int samplex, int sampley, int samplew,
          int sampleh)
{
    ...
    if (noise == NULL)
        noise = TCOD_noise_new(3, TCOD_NOISE_DEFAULT_HURST,
                               TCOD_NOISE_DEFAULT_LACUNARITY, NULL);
    for (x = samplex; x < samplex + samplew; x++) {
        Uint8 *p =
                (Uint8 *) screen->pixels + x * screen->format->BytesPerPixel +
                sampley * screen->pitch;
...
void render_sdl(bool first, TCOD_key_t *key, TCOD_mouse_t *mouse)
{
    if (first) {
        ...
        TCOD_console_set_default_foreground(sample_console, TCOD_white);
        TCOD_console_clear(sample_console);
        TCOD_console_print_rect_ex(sample_console, SAMPLE_SCREEN_WIDTH / 2, 3,
                                   SAMPLE_SCREEN_WIDTH, 0, TCOD_BKGND_NONE,
                                   TCOD_CENTER,
    ...
    if (key->vk == TCODK_TAB) {
        ...
        if (sdl_callback_enabled) {
            ...
            TCOD_console_set_dirty(SAMPLE_SCREEN_X, SAMPLE_SCREEN_Y,
...
int nb_samples =
        sizeof(samples) / sizeof(sample_t); /* total number of samples */
...
int main(int argc, char *argv[])
{
    ...
    for (argn = 1; argn < argc; argn++) {
        if (strcmp(argv[argn], "-font") == 0 && argn + 1 < argc) {
            ...
            font = argv[argn];
            font_flags = 0;
        } else if (strcmp(argv[argn], "-font-nb-char") == 0 &&
                   argn + 2 < argc) {
            argn++;
            ...
            font_flags = 0;
        } else if (strcmp(argv[argn], "-fullscreen-resolution") == 0 &&
                   argn + 2 < argc) {
            argn++;
    ...
    TCOD_console_set_custom_font(font, font_flags, nb_char_horiz,
                                 nb_char_vertic);
    if (fullscreen_width > 0) {
        TCOD_sys_force_fullscreen_resolution(fullscreen_width,
                                             fullscreen_height);
    }
    TCOD_console_init_root(80, 50, "libtcod C sample", fullscreen, renderer);
    /* initialize the offscreen console for the samples */
    sample_console = TCOD_console_new(SAMPLE_SCREEN_WIDTH,
                                      SAMPLE_SCREEN_HEIGHT);
    do {
        if (!credits_end) {
        ...
        for (i = 0; i < nb_samples; i++) {
            ...
            TCOD_console_print_ex(NULL, 2, 46 - (nb_samples - i),
                                  TCOD_BKGND_SET, TCOD_LEFT, samples[i].name);
        }
        ...
        TCOD_console_print(NULL, 2, 47, "%c%c : select a sample",
                           TCOD_CHAR_ARROW_N, TCOD_CHAR_ARROW_S);
        TCOD_console_print(NULL, 2, 48, "ALT-ENTER : switch to %s",
                           TCOD_console_is_fullscreen() ? "windowed mode  "
                                                        : "fullscreen mode");
        ...
        TCOD_console_set_default_foreground(NULL, TCOD_grey);
        TCOD_console_set_default_background(NULL, TCOD_black);
        TCOD_console_print_ex(NULL, 42, 46 - (TCOD_NB_RENDERERS + 1),
                              TCOD_BKGND_SET, TCOD_LEFT, "Renderer :");
        for (i = 0; i < TCOD_NB_RENDERERS; i++) {
            ...
            TCOD_console_print_ex(NULL, 42, 46 - (TCOD_NB_RENDERERS - i),
                                  TCOD_BKGND_SET, TCOD_LEFT, renderer_name[i]);
        }
